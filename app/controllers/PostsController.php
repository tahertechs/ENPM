...
class PostsController extends BaseController{
	...
	public function __construct(){
		...
		$this->beforeFilter('auth', ['except' => ['show','download','index']]);
	}

	//List of all posts open to only poster	
	public function getUserPosts()
	{
		$posts = Post::orderBy('created_at','desc')->where('author_id','=',Auth::user()->id)->paginate(10);
	...
	public function update($id)
	{
		...
		// {
			...
			// 		// return Redirect::route('posts.list')->with('message', 'Succesifully updated!');

			// 	}			
			// }

			// return Redirect::route('posts.list')->with('failure','trying to edit someones else post , BE CAREFULLY BUDDY ! ');
	...
	public function destroy($id)
	{
		...
		if($post){
	        ...
	        foreach ($post->uploads as $photo) {
	            File::delete($filepath . $photo->url);
	            $photo->delete();
	        }
			...
			//Upload::where('post_id', '=', $id)->delete();
			//Post::where('id', '=', $id)->delete();
			Flash::success('Successifully deleted....');
			return Redirect::route('posts.list');			
		}
		return Redirect::route('posts.list');
